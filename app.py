...
class Lesson(db.Model):
    ...
    multimedia_url = db.Column(
        db.String(200), nullable=True
    )  # URL for videos, images, etc.
    ...
    quizzes = db.relationship(
        "Quiz", backref="lesson", lazy=True, cascade="all, delete-orphan"
    )
    assignments = db.relationship(
        "Assignment", backref="lesson", lazy=True, cascade="all, delete-orphan"
    )
    notes = db.relationship(
        "Note", backref="lesson", lazy=True, cascade="all, delete-orphan"
    )
...
class Course(db.Model):
    ...
    progress = db.relationship(
        "Progress", backref="course", lazy=True, cascade="all, delete-orphan"
    )
    enrollments = db.relationship(
...
class Assignment(db.Model):
    ...
    assignment_type = db.Column(
        db.String(20), default="coding"
    )  # coding, text, file_upload
...
class LessonForm(FlaskForm):
    ...
    course_id = StringField(
        "Course ID", validators=[DataRequired()]
    )  # This will be a select field in the template
...
class Grade(db.Model):
    ...
    grade_type = db.Column(
        db.String(20), default="assignment"
    )  # assignment, quiz, exam, participation
...
class ContentApproval(db.Model):
    ...
    )  # ID of the content being approved
    submitted_by = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)
    approved_by = db.Column(
        db.Integer, db.ForeignKey("user.id"), nullable=True
    )  # Approving admin/instructor
...
class ForumThread(db.Model):
    ...
    category_id = db.Column(
        db.Integer, db.ForeignKey("forum_category.id"), nullable=False
    )
    created_at = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)
    ...
    replies = db.relationship(
        "ForumReply", backref="thread", lazy=True, cascade="all, delete-orphan"
    )
...
def enable_2fa():
    ...
    totp_uri = pyotp.totp.TOTP(secret).provisioning_uri(
        name=current_user.email, issuer_name="Java Mastery LMS"
    )
    ...
    return jsonify(
        {"success": True, "qr_code": qr_img, "secret": secret, "uri": totp_uri}
    )
        ...
        if user and bcrypt.check_password_hash(user.password_hash, password):
            ...
            if user.is_2fa_enabled:
                ...
                if not user.totp_secret or not verify_totp_token(
                    user.totp_secret, two_factor_token
                ):
...
def instructor_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if not current_user.is_authenticated or not (
            current_user.is_instructor() or current_user.is_admin()
        ):
            return (
                jsonify({"success": False, "message": "Instructor access required"}),
                403,
            )
...
def student_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if not current_user.is_authenticated or not current_user.is_student():
            return (
                jsonify({"success": False, "message": "Student access required"}),
                403,
            )
        return f(*args, **kwargs)
...
def reset_password(token):
    ...
    if not user:
        return render_template(
            "reset_password.html", error="Invalid or expired reset token"
        )
...
def drop_course(course_id):
    ...
    if not enrollment:
        return (
            jsonify({"success": False, "message": "Not enrolled in this course"}),
            404,
        )
    ...
    if not enrollment:
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Not enrolled in the course containing this lesson",
                }
            ),
            400,
        )
...
def update_enrollment_status(course_id, user_id):
    ...
    if completed_lessons == total_lessons and total_lessons > 0:
        ...
        all_scores = (
            Progress.query.filter_by(user_id=user_id, course_id=course_id)
            .with_entities(Progress.score)
            .all()
        )
...
def quizzes_api():
        ...
        quizzes_data = [
            {
                ...
                "status": "approved",
                "date_created": (
                    q.date_created.isoformat() if hasattr(q, "date_created") else ""
                ),
            }
            for q in quizzes
        ]
    ...
    if not (current_user.is_instructor() or current_user.is_admin()):
        return (
            jsonify(
                {"success": False, "message": "Instructor or admin access required"}
            ),
            403,
        )
    ...
    if not isinstance(options, list) or len(options) < 2:
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Options must be a list with at least 2 items",
                }
            ),
            400,
        )
    ...
    return jsonify(
        {"success": True, "message": "Quiz created successfully", "quiz_id": quiz.id}
    )
        ...
        assignments_data = [
            {
                ...
                "status": "approved",
                "date_created": (
                    a.date_created.isoformat() if hasattr(a, "date_created") else ""
                ),
            }
    ...
    if submissions_count > 0:
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Cannot delete assignment with student submissions",
                }
            ),
            400,
        )
...
def handle_grades():
    if request.method == "POST":
        ...
        # Check permissions
        if current_user.id != user_id and not (
            current_user.is_instructor() or current_user.is_admin()
        ):
            return (
        ...
        if assignment_id:
            ...
            if not assignment:
                return (
                    jsonify({"success": False, "message": "Assignment not found"}),
                    404,
                )
            # Check permission to grade assignment
            if (
                not current_user.is_admin()
                and assignment.course.instructor_id != current_user.id
            ):
                return (
                    jsonify({"success": False, "message": "Insufficient permissions"}),
                    403,
            ...
            if (
                not current_user.is_admin()
                and quiz.lesson.course.instructor_id != current_user.id
            ):
                return (
                    jsonify({"success": False, "message": "Insufficient permissions"}),
                    403,
            ...
            return (
                jsonify(
                    {
                        "success": False,
                        "message": "Either assignment_id or quiz_id must be provided",
                    }
                ),
                400,
            )
...
def get_user_course_grades(user_id, course_id):
    ...
    assignment_grades = [
        g for g in grades if g.assignment_id in [a.id for a in course_assignments]
    ]
    quiz_grades = [g for g in grades if g.quiz_id in [q.id for q in course_quizzes]]

    # Calculate averages
    total_assignment_score = (
        sum(g.grade for g in assignment_grades) if assignment_grades else 0
    )
    total_quiz_score = sum(g.grade for g in quiz_grades) if quiz_grades else 0

    avg_assignment_grade = (
        total_assignment_score / len(assignment_grades) if assignment_grades else 0
    )
            ...
            "assignment_grades": [
                {
                    ...
                    "assignment_title": (
                        Assignment.query.get(g.assignment_id).title
                        if g.assignment_id
                        else "Unknown"
                    ),
                }
            ...
            "quiz_grades": [
                {
                    ...
                    "max_grade": g.max_grade,
                    "quiz_title": (
                        Quiz.query.get(g.quiz_id).title if g.quiz_id else "Unknown"
                    ),
                }
...
def content_approval():
    if request.method == "POST":
        ...
        existing_request = ContentApproval.query.filter_by(
            content_type=content_type, content_id=content_id, status="pending"
        ).first()

        if existing_request:
            return (
                jsonify(
                    {"success": False, "message": "Approval request already exists"}
                ),
                400,
            )
    ...
    for req in approval_requests:
        ...
        request_data = {
            ...
            "date_approved": (
                req.date_approved.isoformat() if req.date_approved else None
            ),
        }
...
def search_content():
    ...
    content_type = request.args.get(
        "type", "all"
    )  # all, course, lesson, assignment, quiz
            ...
            results.append(
                {
                    ...
                    "course_id": lesson.course_id,
                    "course_title": (
                        Course.query.get(lesson.course_id).title
                        if lesson.course_id
                        else "Unknown"
                    ),
            ...
            results.append(
                {
                    ...
                    "course_title": (
                        Course.query.get(assignment.course_id).title
...
def create_note():
    ...
    note = Note(title=title, content=content, lesson_id=lesson_id)
    db.session.add(note)
    db.session.commit()
...
def get_notes_by_lesson(lesson_id):
    ...
    if not (
        ...
        or current_user.is_instructor()
        or Enrollment.query.filter_by(
            user_id=current_user.id, course_id=lesson.course_id
        ).first()
    ):
        return jsonify({"success": False, "message": "Insufficient permissions"}), 403
...
def profile():
    ...
    return render_template(
        "profile.html", progress=user_progress, achievements=user_achievements
    )
...
def dashboard():
    ...
    for course in courses:
        ...
        course.progress_percentage = (
            (completed_lessons / total_lessons * 10) if total_lessons > 0 else 0
        )
        ...
        progress_data.append(
            {
                ...
                "score": record.score,
                "date_completed": (
                    record.date_completed.isoformat() if record.date_completed else None
                ),
            }
        )
...
def check_and_assign_achievements(user_id):
    ...
    if (
        completed_lessons >= 5
        and not Achievement.query.filter_by(user_id=user_id, name="Code Runner").first()
    ):
        achievement = Achievement(
            user_id=user_id, name="Code Runner", description="Completed 5 lessons"
        )
        db.session.add(achievement)

    # Check for "Problem Solver" achievement (average score > 80% on 3 quizzes)
    progress_with_scores = Progress.query.filter(
        Progress.user_id == user_id, Progress.score.isnot(None)
    ).all()

    if len(progress_with_scores) >= 3:
        avg_score = sum(p.score for p in progress_with_scores) / len(
            progress_with_scores
        )
        if (
            avg_score >= 80
            and not Achievement.query.filter_by(
                user_id=user_id, name="Problem Solver"
            ).first()
        ):
            achievement = Achievement(
                user_id=user_id,
...
def submit_quiz():
    ...
    if not quiz_id or not selected_answer:
        return (
            jsonify(
                {"success": False, "message": "Missing quiz_id or selected_answer"}
            ),
            400,
        )
...
def get_lesson_assignment(lesson_id):
    ...
    for assignment in assignments:
        ...
        assignment_data = {
            ...
            "date_submitted": (
                submission.date_submitted.isoformat() if submission else None
            ),
        }
...
def forum_categories():
    ...
    if not (current_user.is_admin() or current_user.is_instructor()):
        return (
            jsonify(
                {"success": False, "message": "Admin or instructor access required"}
            ),
            403,
        )
...
def forum_threads():
    ...
    if request.method == "GET":
        ...
        for thread in threads:
            ...
            latest_reply = (
                max(thread.replies, key=lambda r: r.created_at)
                if thread.replies
                else None
            )
            ...
            thread_data = {
                ...
                "latest_reply_at": (
                    latest_reply.created_at.isoformat() if latest_reply else None
                ),
    ...
    thread = ForumThread(
        title=title, content=content, user_id=current_user.id, category_id=category_id
    )
...
def forum_thread(thread_id):
    ...
    if request.method == "GET":
        ...
        replies = (
            ForumReply.query.filter_by(thread_id=thread_id)
    ...
    if not (
        current_user.id == thread.user_id
...
def forum_replies():
    ...
    if not content or not thread_id:
        return (
            jsonify(
                {"success": False, "message": "Content and thread ID are required"}
            ),
            400,
        )
    ...
    if thread.is_locked and not current_user.is_admin():
        return (
            jsonify(
                {
                    "success": False,
                    "message": "This thread is locked and cannot be replied to",
                }
            ),
            400,
        )
    ...
    return jsonify(
        {"success": True, "message": "Reply added successfully", "reply_id": reply.id}
    )
...
def init_sample_data():
    ...
    try:
        if Course.query.count() == 0:
            ...
            db.session.add_all(
                [quiz1, quiz2, quiz3, quiz4, assignment1, assignment2, assignment3]
            )
            db.session.commit()
    except Exception as e:
...
def add_note(lesson_id):
    ...
    if form.validate_on_submit():
        note = Note(
            title=form.title.data, content=form.content.data, lesson_id=lesson.id
        )
        db.session.add(note)
        db.session.commit()
        return redirect(url_for("admin_dashboard"))
...
def add_assignment(lesson_id):
    ...
    if form.validate_on_submit():
        assignment = Assignment(
            ...
            due_date=(
                datetime.strptime(form.due_date.data, "%Y-%m-%d %H:%M:%S")
...
def edit_assignment(assignment_id):
    ...
    return render_template(
        "admin/edit_assignment.html", form=form, assignment=assignment
    )
