    ...
    try:
        ...
        if completed_lessons >= 5:
            if not Achievement.query.filter_by(
                user_id=user_id, name="Code Runner"
            ).first():
                db.session.add(
                    Achievement(
                        user_id=user_id,
...
def add_lesson(course_id=None):
    form = LessonForm()
    if course_id:
        form.course_id.data = str(course_id)
    if form.validate_on_submit():
        next_num = (
            Lesson.query.filter_by(course_id=int(form.course_id.data)).count() + 1
        )
        lesson = Lesson(
            title=form.title.data,
            ...
            lesson_number=(
                int(form.lesson_number.data) if form.lesson_number.data else next_num
            ),
            multimedia_url=form.multimedia_url.data or None,
            duration=int(form.duration.data) if form.duration.data else 0,
            status="approved",
        )
...
def edit_lesson(lesson_id):
    ...
    if form.validate_on_submit():
        ...
        lesson.course_id = int(form.course_id.data)
        lesson.lesson_number = int(form.lesson_number.data)
        lesson.multimedia_url = form.multimedia_url.data or None
        lesson.duration = int(form.duration.data) if form.duration.data else 0
        lesson.date_updated = datetime.utcnow()
        db.session.commit()
        return redirect(url_for("admin_dashboard"))
    form.course_id.data = str(lesson.course_id)
    form.lesson_number.data = str(lesson.lesson_number)
    form.duration.data = str(lesson.duration)
...
def add_quiz(lesson_id=None):
    ...
    if form.validate_on_submit():
        quiz = Quiz(
            title=form.title.data,
            lesson_id=int(form.lesson_id.data),
            ...
            correct_answer=form.correct_answer.data.upper(),
            points=int(form.points.data) if form.points.data else 1,
            time_limit=int(form.time_limit.data) if form.time_limit.data else 0,
            status="active",
        )
        db.session.add(quiz)
        db.session.commit()
...
def edit_quiz(quiz_id):
    ...
    if form.validate_on_submit():
        ...
        quiz.lesson_id = int(form.lesson_id.data)
        ...
        quiz.correct_answer = form.correct_answer.data.upper()
        quiz.points = int(form.points.data) if form.points.data else 1
        quiz.time_limit = int(form.time_limit.data) if form.time_limit.data else 0
        quiz.date_updated = datetime.utcnow()
        db.session.commit()
        return redirect(url_for("admin_dashboard"))
    form.lesson_id.data = str(quiz.lesson_id)
    form.option_a.data = quiz.options.get("A", "")
    form.option_b.data = quiz.options.get("B", "")
    form.option_c.data = quiz.options.get("C", "")
    form.option_d.data = quiz.options.get("D", "")
    form.points.data = str(quiz.points)
    form.time_limit.data = str(quiz.time_limit)
...
@login_required
@admin_required
def add_assignment(lesson_id=None):
    form = AssignmentForm()
    if lesson_id:
        lesson = Lesson.query.get_or_404(lesson_id)
        form.lesson_id.data = str(lesson_id)
    ...
    if form.validate_on_submit():
        ...
        if form.due_date.data:
            try:
                due_date = datetime.strptime(form.due_date.data, "%Y-%m-%d %H:%M:%S")
            except ValueError:
                try:
        ...
        assignment = Assignment(
            ...
            due_date=due_date,
            max_score=int(form.max_score.data) if form.max_score.data else 100,
            status="active",
        )
...
def edit_assignment(assignment_id):
    ...
    if form.validate_on_submit():
        ...
        assignment.course_id = int(form.course_id.data)
        assignment.lesson_id = int(form.lesson_id.data)
        assignment.assignment_type = form.assignment_type.data
        assignment.due_date = due_date
        assignment.max_score = int(form.max_score.data) if form.max_score.data else 100
        assignment.date_updated = datetime.utcnow()
        db.session.commit()
        return redirect(url_for("admin_dashboard"))
    form.course_id.data = str(assignment.course_id)
    form.lesson_id.data = str(assignment.lesson_id)
    form.max_score.data = str(assignment.max_score)
    if assignment.due_date:
        form.due_date.data = assignment.due_date.strftime("%Y-%m-%d %H:%M:%S")
    return render_template(
...
def add_note(lesson_id=None):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for("admin_dashboard"))
    return render_template("admin/add_note.html", form=form)
...
def edit_note(note_id):
    ...
    if form.validate_on_submit():
        ...
        note.lesson_id = int(form.lesson_id.data)
        note.date_updated = datetime.utcnow()
        db.session.commit()
        return redirect(url_for("admin_dashboard"))
    form.lesson_id.data = str(note.lesson_id)
    ...
    if not expected_key or setup_key != expected_key:
        return (
            jsonify(
                {
                    "success": False,
                    "message": "Unauthorized: Invalid or missing setup key.",
                }
            ),
            401,
        )
    ...
    try:
        ...
        if user:
            ...
            if not admin_email:
                return (
                    jsonify(
                        {"success": False, "message": "Email required for new admin."}
                    ),
                    400,
                )
            user = User(
        ...
        return jsonify(
            {
                "success": True,
                "message": f"Admin user '{admin_user}' {action} successfully.",
            }
        )
